    ...
    <footer className="mt-auto border-t border-neutral-200 bg-white">
      <Container>
        <div className="py-12">
          ...
          {/* Trust Badges */}
          {trustBadges.enabled && trustBadges.items.length > 0 && (
              ...
              <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
                {trustBadges.items.map((badge) => (
                  <div key={badge.id} className="flex flex-col items-center text-center">
                    ...
                    <h5 className="text-sm font-medium text-neutral-900">{badge.title}</h5>
                    <p className="mt-0.5 text-xs text-neutral-600">{badge.description}</p>
                  </div>
                ))}
              </div>
            </div>
